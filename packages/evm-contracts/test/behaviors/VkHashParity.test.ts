...
// Pin every verifier's committed VK_HASH to the manifest generate_verifier.js emits; drift otherwise shows up only as InvalidProof at proof time.
...
describe("VK-hash manifest parity", function () {
  ...
  // The live manifest is regenerated each build; the golden is frozen, so any VK drift fails here.
