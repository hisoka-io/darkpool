...
// suite is fork-only and never runs in CI).
describe("UniswapAdaptor swap-withdraw (mock router, no fork)", function () {
  it("ExactInputSingle: withdraw -> swap -> public memo of the output", async function () {
    ...
    ).deploy(darkPoolAddr, await router.getAddress());
    ...
    expect((await tokenOut.balanceOf(darkPoolAddr)) - before).to.equal(amountIn);
