...
describe("MerkleTreeLib", function () {
  ...
  it("should calculate roots that match the TypeScript implementation", async function () {
    // TS LeanIMT is the source of truth; assert the Solidity harness matches it
  ...
  describe("getSubtreeWithProof", function () {
    it("should return empty proof and single leaf for level 0", async function () {
      ...
      // At level 0 we request a single leaf, so proof spans all the way to root
    ...
    it("should return correct proof and leafs for level 1 subtree", async function () {
      ...
      // Left subtree (position 0) at level 1
      const [proof, leafs] = await harness.getSubtreeWithProof(1, 0);

      // Level 1: 2 leaves + proof to root (TREE_DEPTH - 1)
    ...
    it("should return correct proof and leafs for right subtree", async function () {
      ...
      // Right subtree (position 1) at level 1
    ...
    it("should return zeros for empty leaf positions", async function () {
      ...
      // Right subtree (position 1) at level 1 is unpopulated
    ...
    it("should return all leafs at maximum level", async function () {
      ...
      // At level TREE_DEPTH the whole leaf array is returned (2^TREE_DEPTH = 16 for depth 4)
      const [proof, leafs] = await harness.getSubtreeWithProof(TREE_DEPTH, 0);

      expect(proof.length).to.equal(0); // No proof needed at root level
      expect(leafs.length).to.equal(1 << TREE_DEPTH);
    ...
    it("should return correct subtree for level 2", async function () {
      ...
      // Second subtree (position 1) at level 2
      ...
      expect(leafs[0]).to.equal(ethers.zeroPadValue(ethers.toBeHex(5), 32));
      expect(leafs[1]).to.equal(ethers.zeroPadValue(ethers.toBeHex(6), 32));
      expect(leafs[2]).to.equal(ethers.zeroPadValue(ethers.toBeHex(7), 32));
      expect(leafs[3]).to.equal(ethers.zeroPadValue(ethers.toBeHex(8), 32));
    });
    ...
    it("should return correct multiple subtrees from same level", async function () {
      ...
      // All subtrees at level 2 (4 subtrees, each with 4 leaves)
      const subtrees = [];
      for (let pos = 0; pos < 4; pos++) {
        const [proof, leafs] = await harness.getSubtreeWithProof(2, pos);
      ...
      // First subtree holds leaves 1-4
      for (let j = 0; j < 4; j++) {
      ...
      // Second subtree holds leaves 5-8
      for (let j = 0; j < 4; j++) {
