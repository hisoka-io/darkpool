...
/// Poseidon2 permutation & hash primitives implemented in pure Yul assembly.
/// All functions are internal + pure so the assembly is inlined at every call.
library LibPoseidon2Yul {
    uint256 internal constant PRIME =
        0x30644e72e131a029b85045b68181585d2833e84879b9709143e1f593f0000001;

    function hash_1(uint256 x) internal pure returns (uint256 result) {
        (result, , , ) = permutation(x, 0, 0, 1 << 64);
    }

    function hash_2(uint256 x, uint256 y) internal pure returns (uint256 result) {
        (result, , , ) = permutation(x, y, 0, 2 << 64);
    }

    function hash_3(uint256 x, uint256 y, uint256 z) internal pure returns (uint256 result) {
        (result, , , ) = permutation(x, y, z, 3 << 64);
    }

    /// @notice Full Poseidon2 permutation returning the complete 4-element state.
    /// @dev Used by the variable-length sponge to chain permutations while
    ///      preserving the full internal state between absorb steps.
    function permutation(uint256 s0, uint256 s1, uint256 s2, uint256 s3)
        internal
        pure
        returns (uint256 out0, uint256 out1, uint256 out2, uint256 out3)
    {
        assembly {
...

            {
                ...
                t2 := addmod(t2, t1, P) // clean
                let t3 := add(state3, state3)
                t3 := addmod(t3, t0, P) // clean
                let t4 := mulmod(t1, 4, P)
                t4 := add(t4, t3) // dirty 1/3
                let t5 := mulmod(t0, 4, P)
                t5 := add(t5, t2) // dirty 1/3
                let t6 := addmod(t3, t5, P) // clean
                let t7 := addmod(t2, t4, P) // clean
                state0 := t6 // clean
                state1 := t5 // dirty 1/3
                state2 := t7 // clean
                state3 := t4 // dirty 1/3
            ...
            // Round 0 (external)
            {
...

                {
                    // single_box
                    let intr := state0
                    state0 := mulmod(intr, intr, P)
                    state0 := mulmod(state0, state0, P)
                    state0 := mulmod(state0, intr, P)
                }

                {
                    // single_box
                    let intr := state1
                    state1 := mulmod(intr, intr, P)
                    state1 := mulmod(state1, state1, P)
                    state1 := mulmod(state1, intr, P)
                }

                {
                    // single_box
                    let intr := state2
                    state2 := mulmod(intr, intr, P)
                    state2 := mulmod(state2, state2, P)
                    state2 := mulmod(state2, intr, P)
                }

                {
                    // single_box
                    let intr := state3
                    state3 := mulmod(intr, intr, P)
                    state3 := mulmod(state3, state3, P)
                    state3 := mulmod(state3, intr, P)
                }
            ...
            // Round 3 (external)
            {
...

                {
                    ...
                    t3 := addmod(t3, t0, P) // clean
                    let t4 := mulmod(t1, 4, P)
                    t4 := addmod(t4, t3, P) // clean
                    let t5 := mulmod(t0, 4, P)
                    t5 := addmod(t5, t2, P) // clean
                    let t6 := addmod(t3, t5, P) // clean
                    let t7 := addmod(t2, t4, P) // clean
                    state0 := t6 // clean
                    state1 := t5 // clean
            ...
            // Round 4 (internal)
            {
...

                {
                    ...
                    sum := add(sum, state2)
                    sum := addmod(sum, state3, P)
                    state0 := addmod(
                        mulmod(state0, 0x10dc6e9c006ea38b04b1e03b4bd9490c0d03f98929ca1d7fb56821fd19d3b6e7, P),
                        sum,
                        PRIME
                    )
                    state1 := addmod(
                        mulmod(state1, 0x0c28145b6a44df3e0149b3d0a30b3bb599df9756d4dd9b84a86b38cfb45a740b, P),
                        sum,
                        PRIME
                    )
                    state2 := addmod(
                        mulmod(state2, 0x00544b8338791518b2c7645a50392798b21f75bb60e3596170067d00141cac15, P),
                        sum,
                        PRIME
                    )
                    state3 := addmod(
                        mulmod(state3, 0x222c01175718386f2e2e82eb122789e352e105a3b8fa852613bc534433ee428b, P),
                        sum,
                        PRIME
                    )
                }
